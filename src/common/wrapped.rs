...
// This is just Rc wrapper that implements equivalance and hash
// based on pointer address

pub struct RcEqWrapper<T> {
    inner: Rc<T>,
}

impl<T> RcEqWrapper<T> {
    pub fn new(value: Rc<T>) -> Self {
        RcEqWrapper { inner: value }
    }
...
impl<T> Clone for RcEqWrapper<T> {
    fn clone(&self) -> Self {
        RcEqWrapper {
            inner: self.inner.clone(),
        }
...
impl<T> PartialEq for RcEqWrapper<T> {
    fn eq(&self, other: &Self) -> bool {
        Rc::ptr_eq(&self.inner, &other.inner)
    }
}

impl<T> Eq for RcEqWrapper<T> {}
