...
use crate::protocol::workermsg::{ComputeTaskMsg, FromWorkerMessage, Status, TaskFinishedMsg, AddKeysMsg};
...
pub fn compute_task(state_ref: &WorkerStateRef, mut msg: ComputeTaskMsg) -> crate::Result<()> {
    ...
    let fetched_keys : Vec<_> = std::mem::take(&mut msg.who_has).into_iter()
        .map(|(k, _)| k)
    ...
    if !fetched_keys.is_empty() {
        ...
        state.send(serialize_single_packet(
            FromWorkerMessage::<SerializedTransport>::AddKeys(AddKeysMsg {
                keys: fetched_keys
            })
        )?);
    }
    state.local_keys.insert(msg.key.clone());
    state.send(serialize_single_packet(FromWorkerMessage::<SerializedTransport>::TaskFinished(
            TaskFinishedMsg {
                status: Status::Ok,
                key: msg.key,
                nbytes: 20,
